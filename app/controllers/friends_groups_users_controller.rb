class FriendsGroupsUsersController < ApplicationController
  ...
  def create
    ...
    if @friends_groups_user.save
      render :show, status: @friends_groups_user_is_new_record ? :created : :ok, location: friends_group_friend_url(@friends_group, @friends_groups_user)
    else
